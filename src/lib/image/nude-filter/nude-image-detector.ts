...
export class NudeImageDetector {

    detect(file:File){
      let url = URL.createObjectURL(file);
      return new Promise<boolean>((resolve)=> {
        loadImage(url).then(image=>{
          this.process(image,(result:boolean)=>{
            resolve(result);
          });
        })
      });
    ...
    private process(img:Image,callback:any){
      ...
      lastTo = -1,
      totalSkin = 0,
      addMerge = function(from:any, to:any) {
        lastFrom = from;
        lastTo = to;
    ...
    classifySkin(r:any, g:any, b:any) {
      ...
      toNormalizedRgb(r:any, g:any, b:any) {
          var sum = r + g + b;
